...
class JsonPlaceHolderControllerTest {
    ...
    @Test
    fun `should response status 201 for POST request`() {
        ...
        every { mockJsonPlaceHolderService.savePost(jsonPlaceHolder) } returns null

        mockMvc.perform(
            post("/api/v1/posts")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(jsonPlaceHolder) )
        )
            .andExpect(status().isCreated)
    }

    @Test
    fun `should invoke JsonPlaceHolderService savePost method and return response`() {
        val jsonPlaceHolder = JsonPlaceHolder(
            id = 2,
            slug = "lorem-ipsum",
            url = "https://jsonplaceholder.org/posts/lorem-ipsum",
        ...
        every { mockJsonPlaceHolderService.savePost(jsonPlaceHolder) } returns jsonPlaceHolder
